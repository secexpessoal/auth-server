...
import type { PaginatedResponseDto, RegisterRequestDto, RegisterResponseDto } from "@lib/data/manager/molecule/user.types";
...
export async function registerAdminAttempt(payload: RegisterRequestDto): Promise<RegisterResponseDto> {
  const { data } = await axiosClient.post<RegisterResponseDto>("/v2/user/register/admin", payload);
  return data;
}
...
export async function getUsersList(
  ...
  const { data } = await axiosClient.get<PaginatedResponseDto<UserResponseDto>>(`/v2/user`, {
