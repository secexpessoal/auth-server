...
function InputGroup({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      role="group"
      className={cn(
        "group/input-group relative flex w-full min-w-0 items-center h-12 rounded-2xl border border-black/5 dark:border-white/10 bg-white/60 dark:bg-white/5 backdrop-blur-md transition-all duration-300 has-[[data-slot=input-group-control]:focus]:border-primary/40 has-[[data-slot][aria-invalid=true]]:border-destructive/50 overflow-hidden",
        className,
      )}
      {...props}
    />
...
function InputGroupInput({ className, ...props }: React.ComponentProps<"input">) {
  ...
  return (
    <Input
      ...
      className={cn(
        "h-full rounded-none border-0 bg-transparent shadow-none ring-0 focus-visible:ring-0 focus:bg-transparent disabled:bg-transparent aria-invalid:ring-0 dark:bg-transparent dark:disabled:bg-transparent flex-1 px-3 py-0 leading-normal",
